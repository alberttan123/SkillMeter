            ...
            <div id="section2">
                ...
                <div class="select-container">
                    <select class="select-box" id="subject" onchange="change()">
                        ...
                        $userCountsql = "SELECT UID, username FROM USER_INFO";
                        $sqlUserInfo = query($userCountsql);
                        $userCountDB = mysqli_num_rows($sqlUserInfo);

                        for($userCount = 0; $userCount < $userCountDB; $userCount++){
                        $row = mysqli_fetch_row($sqlUserInfo);
                        $UIDRow = $row[0];
                        $usernameRow = $row[1];
                        echo "<option value='$UIDRow'>$usernameRow</option>";}?>
                    </select>
